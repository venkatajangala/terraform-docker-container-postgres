...
# Read a seeded secret using the root token (written to .vault-bootstrap/vault-init.json on first deploy)
VAULT_TOKEN=$(jq -r '.root_token' .vault-bootstrap/vault-init.json 2>/dev/null)
