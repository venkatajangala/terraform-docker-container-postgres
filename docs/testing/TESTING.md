...
Comprehensive testing procedures and validation for PostgreSQL HA + PgBouncer infrastructure.
...
PGPASSWORD='pgAdmin1' psql -h localhost -p 6432 -U pgadmin -d postgres -c "SELECT version();"
...
```bash
docker exec pgbouncer-2 bash -c "PGPASSWORD='pgAdmin1' psql -h localhost -p 6432 -U pgadmin -d postgres -c \"SELECT version();\""
```
**Expected Output:**
```
PostgreSQL 18.2 (Debian 18.2-1.pgdg13+1) on x86_64-pc-linux-gnu, compiled by gcc...
...
```bash
docker exec pgbouncer-1 bash -c "PGPASSWORD='pgAdmin1' psql -h localhost -p 6432 -U pgadmin -d pgbouncer -c \"SHOW POOLS;\""
```
**Expected Output:**
```
...
```bash
docker exec pgbouncer-1 bash -c "PGPASSWORD='pgAdmin1' psql -h localhost -p 6432 -U pgadmin -d pgbouncer -c \"SHOW STATS;\""
```
**Expected Output:**
```
...
```bash
PGPASSWORD='pgAdmin1' psql -h localhost -p 6432 -U pgadmin -d postgres <<EOF
...
User: pgadmin
Password: pgAdmin1 (plain text in config file)
PgBouncer Port: 6432
```
...
# Method 3: One-liner
PGPASSWORD='pgAdmin1' psql -h localhost -p 6432 -U pgadmin -d postgres
```

**For Docker exec:**
```bash
docker exec pgbouncer-1 bash -c "PGPASSWORD='pgAdmin1' psql -h localhost -p 6432 -U pgadmin -d postgres -c \"<query>\""
```
...
docker exec pgbouncer-1 bash -c "PGPASSWORD='pgAdmin1' psql -h localhost -p 6432 -U pgadmin -d postgres -c 'SELECT 1;'"
```
...
**Test Summary:** 17/17 automated tests passed ✅
